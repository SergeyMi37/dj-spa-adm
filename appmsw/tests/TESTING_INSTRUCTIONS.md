...
```bash
7. `ParamCRUDTestCase` - Тестирование CRUD операций для Param
8. `ParamPermissionsTestCase` - Тестирование прав доступа для Param
9. `ParamSwaggerDocumentationTestCase` - Тестирование документации Swagger для Param
python manage.py test appmsw.tests.db_connection_tests
```

10. `CommentCRUDTestCase` - Тестирование CRUD операций для Comment
11. `CommentPermissionsTestCase` - Тестирование прав доступа для Comment
12. `CommentSwaggerDocumentationTestCase` - Тестирование документации Swagger для Comment
### 3. Запуск только тестов SysOption

```bash
python manage.py test appmsw.tests.sys_option_tests
```

### 4. Запуск конкретного теста
```bash
python manage.py test appmsw.tests.param_tests
```bash
python manage.py test appmsw.tests.comment_tests
```
```

```bash
python manage.py test appmsw.tests.sys_option_tests.SysOptionCRUDTestCase.test_create_sys_option
```
...
```bash
pytest appmsw/tests/db_connection_tests.py
pytest appmsw/tests/sys_option_tests.py
```
...
- `test_create_param` - Проверяет создание нового параметра
- `test_get_param_list` - Проверяет получение списка параметров
- `test_get_single_param` - Проверяет получение одного параметра
- `test_update_param` - Проверяет полное обновление параметра
- `test_partial_update_param` - Проверяет частичное обновление параметра
- `test_delete_param` - Проверяет удаление параметра

### ParamPermissionsTestCase

- `test_user_can_access_own_param` - Проверяет, что пользователь может получить доступ к своим параметрам
- `test_user_cannot_access_other_user_param` - Проверяет, что пользователь не может получить доступ к чужим параметрам
- `test_unauthenticated_access_denied` - Проверяет, что неавторизованный доступ запрещен
### CommentCRUDTestCase

- `test_create_comment` - Проверяет создание нового комментария
- `test_get_comment_list` - Проверяет получение списка комментариев
- `test_get_single_comment` - Проверяет получение одного комментария
- `test_update_comment` - Проверяет полное обновление комментария
- `test_partial_update_comment` - Проверяет частичное обновление комментария
- `test_delete_comment` - Проверяет удаление комментария

### CommentPermissionsTestCase

- `test_user_can_access_own_comment` - Проверяет, что пользователь может получить доступ к своим комментариям
- `test_user_cannot_access_other_user_comment` - Проверяет, что пользователь не может получить доступ к чужим комментариям
- `test_unauthenticated_access_denied` - Проверяет, что неавторизованный доступ запрещен

### CommentSwaggerDocumentationTestCase

- `test_swagger_schema_available` - Проверяет доступность схемы Swagger
- `test_swagger_ui_available` - Проверяет доступность интерфейса Swagger
- `test_comment_in_swagger_schema` - Проверяет, что Comment API присутствует в схеме Swagger

### ParamSwaggerDocumentationTestCase

- `test_swagger_schema_available` - Проверяет доступность схемы Swagger
- `test_swagger_ui_available` - Проверяет доступность интерфейса Swagger
- `test_param_in_swagger_schema` - Проверяет, что Param API присутствует в схеме Swagger
